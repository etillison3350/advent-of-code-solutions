...
from helpers import input_parse as ip


class StrInputExecutor:
    ...
    def solve(self, input_lines: ip.StrInput, print: Callable[..., None]) -> Generator[Any, None, None]:
        pass

    def _compute_true_output(self, input_lines: ip.StrInput, output_pipe: Connection) -> None:
        answers = []
    ...
    def _compute_test_cases(self, test_cases: Sequence[str], true_output_pipe: Connection) -> None:
        true_answers = []

        for index, tc in enumerate(test_cases):
            tc_inp = ip.StrInput(tc)
            tc_str = str(tc_inp.splitlines().native())
            ...
            try:
                # Run the test case
                for part, ans in enumerate(self.solve(tc_inp, tc_print)):
    ...
    def execute(self, use_cached_test_cases=True):
        input.wait_for_input(self.day, self.year)

        recv_pipe, send_pipe = Pipe(False)

        inp = input.input_text(self.day, self.year)
        str_inp = ip.StrInput(inp)

        true_output_proc = Process(target=self._compute_true_output, args=(str_inp, send_pipe))
        true_output_proc.start()

        test_cases = input.find_test_cases(self.day, self.year, cached=use_cached_test_cases)
        test_case_proc = Process(target=self._compute_test_cases, args=(test_cases, recv_pipe))
...
class SplitExecutor:
    ...
    def execute(self, split_seq: Optional[str] = '\n', use_cached_test_cases=True):
        input.wait_for_input(self.day, self.year)

        recv_pipe, send_pipe = Pipe(False)

        inp = input.input_text(self.day, self.year)
        input_lines = [inp] if split_seq is None else inp.split(split_seq)
