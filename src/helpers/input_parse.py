...
class StrInput(BasicInput[str]):
    ...
    def split(self, split_seq: Optional[str] = r'\s', max_split: int = 0, flags: int | re.RegexFlag = 0, remove_empty=False) -> StrListInput:
    ...
    def sections(self, *classes: str, include_no_match=True, remove_empty=True, opt_int=True) -> StrListInput | ListInput[str | int]:
        if len(classes) == 0:
            classes = ['A-Za-z', '0-9']
        return self.extract(*[f'[{c}]+' for c in classes], include_no_match=include_no_match, remove_empty=remove_empty, opt_int=opt_int)

    def extract(self, *patterns: str, include_no_match=True, remove_empty=True, opt_int=False) -> StrListInput | ListInput[str | int]:
...
class StrListInput(ListInput[str]):
    ...
    def split(self, split_seq: str, max_split: int = 0, flags: int | re.RegexFlag = 0, remove_empty=False) -> ListInput[StrListInput]:
        return ListInput(s.split(split_seq, max_split, flags, remove_empty) for s in self._inp)

    def sections(self, *classes: str, include_no_match=True, remove_empty=True, opt_int=True) -> ListInput[ListInput[str | int]]:
    ...
    def extract(self, *patterns: str, include_no_match=True, remove_empty=True, opt_int=False) -> ListInput[ListInput[str | int]]:
